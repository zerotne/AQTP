...
class Transformer_dec(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        self.vl_target_decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                                    return_intermediate=return_intermediate_dec)

        self.self_attn_vl = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
    ...
    def forward(self, feat, text_query_token, t_l_pos_embed, visual_query_token, t_v_pos_embed, tgt_pre, pos,
                query_pos):

        memory = feat  # output of hivit

        tgt_q = self.self_attn_vl(query=self.with_pos_embed(visual_query_token, t_v_pos_embed),
                                  key=self.with_pos_embed(text_query_token, t_l_pos_embed),
                                  value=text_query_token, attn_mask=None, key_padding_mask=None)[0]
        tgt_kv = tgt_pre.copy()
        tgt_kv.append(tgt_q)
        tgt_kv = torch.cat(tgt_kv, dim=0)
        tgt = [tgt_q, tgt_kv]
        hs = self.vl_target_decoder(tgt, memory, pos=pos, query_pos=query_pos)
        return hs
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt_all, memory,
        ...
        tgt = self.norm3(tgt)
        # 更新tgt_all中的tgt_q
        tgt_all[1] = tgt_all[1][:-1, :, :]
        tgt_all[1] = torch.cat([tgt_all[1], tgt], dim=0)
    ...
    def forward(self, tgt, memory,
           ...
           # return self.forward_pre(tgt, memory, tgt_mask, memory_mask,
                                    #tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos)  # pos
        output = self.forward_post(tgt, memory, tgt_mask, memory_mask,
                                 tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos)     # pos
        return output
