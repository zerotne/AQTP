...
from lib.models.AQTP.base_backbone import BaseBackbone
from ..layers.target_query_prompt import TQP
...
class HiViT(BaseBackbone):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, prompt_layers=8,
        ...
        prompt_blocks = []
        for i in range(self.prompt_layers):
            prompt_blocks.append(TQP(v_dim=self.embed_dim))
        self.rgbl_prompt_blocks = nn.Sequential(*prompt_blocks)


        self.rgbl_patch_embed16 = PatchEmbed1(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=self.embed_dim)

        self.rgbl_tp = nn.Linear(self.embed_dim, self.embed_dim, bias=False)
        # build blocks
